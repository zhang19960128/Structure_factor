...
def factorsum(MDsnap,period,ScatteringF,Qpoints,cell):
    ...
    rank = comm.Get_rank();
    size = comm.Get_size();
    PeriodND=np.ones(np.shape(MDsnap));
    for k in range(3):
        PeriodND[:,k]=PeriodND[:,k]*period[k];
    for i in range(rank,natom,size):
        reciprocal=np.array([2*np.pi,2*np.pi,2*np.pi])/(period/cell);
        posit=np.ones(np.shape(MDsnap));
        for k in range(3):
            posit[:,k]=posit[:,k]*MDsnap[i,k];
        dp=posit-MDsnap-np.round((posit-MDsnap)/PeriodND)*PeriodND;
        value=np.matmul(dp,np.transpose(Qpoints*reciprocal));
        value=np.exp(-1j*value);
        sumvalue=sumvalue+np.sum(value);
    divisor=0.0;
